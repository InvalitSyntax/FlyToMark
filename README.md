...
Воспользуемся методом полёта `set_velocity()`:[справка](https://clover.coex.tech/ru/simple_offboard.html#setvelocity)

Кратко: задаём скорость смещения в `м/с` по x, y и z координатам -> дрон летит по результирующему вектору
